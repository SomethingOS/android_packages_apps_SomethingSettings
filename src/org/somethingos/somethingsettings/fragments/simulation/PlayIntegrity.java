...
public class PlayIntegrity extends SettingsPreferenceFragment implements Preference.OnPreferenceChangeListener {
    ...
    private static final String BLOCK_KEY_ATTEST = "persist.sys.sih.block_ecc";
    private static final String USE_INTEGRITY_SPOOF = "persist.sys.sih.integrity_spoof";

    @Override
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference.getKey().equals("play_integrity_switch")) {
            boolean enabled = (Boolean) newValue;
            SystemProperties.set(USE_INTEGRITY_SPOOF, enabled ? "true" : "false");
            killGmsProcess();
            return true;
        } else if (preference.getKey().equals("play_integrity_tricky_switch")) {
            boolean enabled = (Boolean) newValue;
            SystemProperties.set(BLOCK_KEY_ATTEST, enabled ? "true" : "false");
            killGmsProcess();
            return true;
        }
        return false;
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        if (playIntegritySwitch != null) {
            boolean isEnabled = SystemProperties.getBoolean(USE_INTEGRITY_SPOOF, true);
            playIntegritySwitch.setChecked(isEnabled);
          
            playIntegritySwitch.setOnPreferenceChangeListener(this);
        }

        if (playIntegrityTrickySwitch != null) {
            boolean isEnabled = SystemProperties.getBoolean(BLOCK_KEY_ATTEST, false);
            playIntegrityTrickySwitch.setChecked(isEnabled);
            playIntegrityTrickySwitch.setOnPreferenceChangeListener(this);
        }
    }
